...
export async function PUT(request: Request) {
  try {
    ...
    if (existingLike) {
      await prisma.vortex_CommentLike.delete({
        where: {
          userId_commentId: {
            commentId: postInfo.data.id,
            userId: user.id,
    ...
    if (!existingDislike) {
      await prisma.vortex_CommentLike.create({
        data: {
          commentId: postInfo.data.id,
          userId: user.id,
        },
    ...
    await prisma.$transaction([
      prisma.vortex_CommentDisLike.delete({
        where: {
          userId_commentId: {
            commentId: postInfo.data.id,
            userId: user.id,
      ...
      prisma.vortex_CommentLike.create({
        data: {
          commentId: postInfo.data.id,
          userId: user.id,
        },
    ...
    return NextResponse.json({ message: "Liked the Comment" }, { status: 200 });
  } catch (err: any) {
    return NextResponse.json(
